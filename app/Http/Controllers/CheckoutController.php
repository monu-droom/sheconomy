...
class CheckoutController extends Controller
{
    ...
    {
        if (Auth::check()) {
            if (isset($request->address_id) == null) {
        ...
        //Shipping Cost 
        $cart_exist = Cart::where('user_id', Auth::id())
                        ->where('product_id', $request->product_id)
                        ->first();
        $carts = Cart::findOrFail($cart_exist->id);
        if($carts){
            ...
            $carts->shipping_cost = !empty($shipping_price) ? $shipping_price : '';
            $carts->save();
        }    
        $subtotal = 0;
        $tax = 0;
    ...
    public function store_delivery_info(Request $request)
    {
        if (Session::has('cart') && count(Session::get('cart')) > 0) {
            ...
            $seller_user_id = User::where('id', $seller->user_id)->first();
            $shop = Shop::where('user_id', $seller_user_id->id)->first();
...
//            });
            ...
            //dd($total);
            return view('frontend.payment_select', compact('total', 'shop', 'seller'));
        } else {
    ...
    public function get_payment_info(Request $request)
    {
        ...
        return view('frontend.payment_select', compact('total', 'shop'));
    }
