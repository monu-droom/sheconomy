...
class StripeController extends Controller
{
    ...
    public function create_checkout_session(Request $request)
    {
        ...
        if(!empty($request->order_id)){
            $order_detail = \App\OrderDetail::where('order_id', $request->order_id)->first();
        }else{
            $order_detail = '';
        }
        if(!empty($order_detail)){
            $products = \App\Product::findOrFail($order_detail->product_id);
        }else{
            $products = '';
        }
        if(!empty($products)){
            $seller = \App\Seller::where('user_id', $products->user_id)->first();
        }else{
            $seller = '';
        }
        if(!empty($seller)){
            $seller_payment = \App\SellerPaymentSetting::where('seller_id', $seller->id)->first();
        }else{
            $seller_payment = '';            
        }
        if(!empty($seller_payment) && $seller_payment->stripe_status == 1){
            $stripe_key = $seller_payment->stripe_secret;
        }else{
            $stripe_key = '';
        }
