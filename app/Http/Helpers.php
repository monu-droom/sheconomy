...
//convert INR to USD
if (! function_exists('convert_inr_to_usd')) {
    function convert_inr_to_usd($amount) {
...
if (! function_exists('discountPrice')) {
    function discountPrice($id) {
        if(!empty($id)){
            ...
            $variant_product = ProductStock::where('product_id', $id)->first();
            $discount_type = !empty($product->discount_type) ? $product->discount_type : '';
            $discount = !empty($product->discount) ? $product->discount : '';
            if(!$variant_product){                
                if($discount_type == 'percent'){
                    $discount_price = ($product->unit_price * $product->discount)/100;
                    $discount_price = single_price($discount_price);
                }else{
                    $discount_price = single_price($discount);
                }
            }else{                
                if($discount_type == 'percent'){
                    $discount_price = ($variant_product->price * $product->discount)/100;
                    $discount_price = single_price($discount_price);
                }else{
                    $discount_price = single_price($discount);
                }
...
if (! function_exists('format_price')) {
    function format_price($price)
    {
        if(BusinessSetting::where('type', 'symbol_format')->first()->value == 1){
            return currency_symbol().number_format((float)$price, BusinessSetting::where('type', 'no_of_decimals')->first()->value);
        }
...
if (! function_exists('home_price')) {
    function home_price($id)
    {
        ...
        if(strtolower($code) == 'rupee'){
            ...
            $lowest_price = $product->price_usd;            
        }
        // $highest_price = $product->unit_price;
        if($product->variant_product > 0){
            $product_stock = ProductStock::where('product_id', $product->id)->get();
            foreach($product_stock as $key => $stock){                                
                if(strtolower($code) == 'rupee'){                    
                    if($lowest_price > $stock->price){
                        $lowest_price = $stock->price;
                    }
                }else{
                    if($lowest_price > $stock->price_usd){
                        $lowest_price = $stock->price_usd;
                    }
                }
        ...
        // }
        // elseif($product->tax_type == 'amount_usd'){
        //     $lowest_price += $product->tax;
        //     $highest_price += $product->tax;
        // }elseif($product->tax_type == 'amount_inr'){
        //     $lowest_price += $product->tax;
        //     $highest_price += $product->tax;            
        // }
...
if (! function_exists('home_discounted_price')) {
    function home_discounted_price($id)
    {
        ...
        if (!$inFlashDeal) {
            ...
            elseif($product->discount_type == 'amount_usd'){
...
if (! function_exists('homePrice')) {
    function homePrice($id)
    {
        ...
        if(strtolower($code) == 'rupee'){
            ...
            $lowest_price = $product->price_usd;
            $highest_price = $product->price_usd;
        }
