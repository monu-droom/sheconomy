	...
	<div style="margin-left:auto;margin-right:auto;">
		...
		<div style="background: #eceff4;padding: 1.5rem;">
			<table>
				<tr>
					<td>
                                            <img loading="lazy"  src="http://localhost/public/uploads/logo/yUaIaIFLEAwUR1iSZf3QEFYa3JvLrpSKBBphRe4b.jpeg" height="40" style="display:inline-block;">
					</td>
			...
			<table>
                            <?php 
                                $user_from_seller = App\Seller::where('id', $order_details->seller_id)->first();
                            ...
                            ?>
				<tr>
					<td style="font-size: 1.2rem;" class="strong">{{ $shop->name }}</td>
					<td class="text-right"></td>
				</tr>
				<tr>
					<td class="gry-color small">{{ $shop->address }}</td>
					<td class="text-right"></td>
				</tr>
				<tr>
					<td class="gry-color small">{{ translate('Email') }}: {{ $user->email }}</td>
					<td class="text-right small"><span class="gry-color small">{{ translate('Order ID') }}:</span> <span class="strong">{{ $order->code }}</span></td>
				</tr>
				<tr>
					<td class="gry-color small">{{ translate('Phone') }}: {{ $user->phone }}</td>
				...
				<thead>
	                <tr class="gry-color" style="background: #eceff4;">
	                    ...
	                    <th width="15%">{{ translate('Unit Price') }}</th>
                            <!-- calculating tax for indian seller -->
                            <?php 
                            ...
                            <th width="10%">{{ translate('Cgst') }}</th>
                            <th width="10%">{{ translate('Sgst') }}</th>
                            @else
                            <th width="10%">{{ translate('Igst') }}</th>
                            @endif
                            @else
		                ...
		                @if ($orderDetail->product != null)
							<tr class="">
								...
								<td class="gry-color currency">{{ single_price($orderDetail->price/$orderDetail->quantity) }}</td>
                                                                @if(strtolower($shop->country) == 'india')
                                                                @if(strtolower($current_user->city) == strtolower($shop->city))
                                                                <td class="gry-color currency">{{ single_price($cgst/$orderDetail->quantity) }}</th>
                                                                <td class="gry-color currency">{{ single_price($sgst/$orderDetail->quantity) }}</th>
                                                                @else
	        ...
	        <table style="width: 40%;margin-left:auto;" class="text-right sm-padding small strong">
		        <tbody>
			        ...
			        <tr class="border-bottom">
			            ...
			            <td class="currency">{{ single_price($cgst/$orderDetail->quantity) }}</th>
			        </tr>
			        <tr class="border-bottom">
			            <th class="gry-color text-left">{{ translate('Sgst') }}</th>
			            <td class="currency">{{ single_price($sgst/$orderDetail->quantity) }}</th>
			        </tr>
                                @else
			        <tr class="border-bottom">
			            ...
			            <td class="currency">{{ single_price($orderDetail->tax/$orderDetail->quantity) }}</td>
                                ...
                                @else
			        <tr class="border-bottom">
			            <th class="gry-color text-left">{{ translate('Tax') }}</th>
			            <td class="currency">{{ single_price($orderDetail->tax/$orderDetail->quantity) }}</td>
			        </tr>
                                @endif
			        <tr>
