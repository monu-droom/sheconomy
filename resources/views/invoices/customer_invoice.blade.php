				...
				<thead>
	                <tr class="gry-color" style="background: #eceff4;">
	                    ...
	                    <th width="15%">{{ translate('Unit Price') }}</th>
                            ...
                            @endif
	                    <th width="15%" class="text-right">{{ translate('Shipping Cost') }}</th>
	                ...
	                @foreach ($order->orderDetails as $key => $orderDetail)
		                @if ($orderDetail->product != null)
                                    <tr class="">
                                        ...
                                        @if(strtolower($shop->country) == 'india')
                                        @if(strtolower($current_user->city) == strtolower($shop->city))
                                        <td class="gry-color currency">{{ single_price($cgst) }}</th>
                                        <td class="gry-color currency">{{ single_price($sgst) }}</th>
                                        @else
										...
										<?php $order->grand_total = $order->orderDetails->sum('shipping_cost') + $order->orderDetails->sum('price'); ?>
                                        <td class="text-right currency">{{ single_price($order->grand_total ) }}</td>
                                    </tr>
		                @endif
			@endforeach
	            </tbody>
