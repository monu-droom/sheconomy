    ...
    <div class="card-body">
                ...
                @foreach (Session::get('cart') as $key => $cartItem)
                    @php
                        ...
                        $carts = \App\Cart::where('user_id', Auth::id())
                                            ...
                                            ->first();
                        $user = Auth::user();
                        $user = \App\Address::where('user_id', $user->id)->first();
                        $shop = \App\Shop::where('user_id', $product->user_id)->first();
                        $subtotal += ($carts->price - $carts->tax) * $carts->quantity;
                        $tax += $carts->tax * $carts->quantity;
                        $cgst = $tax / 2;
                        $sgst = $tax - $cgst;
                    ...
                    <tr class="cart_item">
                        <td class="product-name">
                            ...
                            <strong class="product-quantity">× {{ $carts->quantity }}</strong>
                        </td>
                        <td class="product-total text-right">
                            <span class="pl-4">{{ single_price(($carts->price - $carts->tax) * $carts->quantity) }}</span>
                        </td>
                    </tr>
                @endforeach
            </tbody>
        ...
        <table class="table-cart table-cart-review">
            ...
            <tfoot>
                ...
                @if(strtolower(isset($user->city)) == strtolower(isset($shop->city)))
