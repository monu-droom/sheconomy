                                    ...
                                    <tbody>
                                        ...
                                        @foreach (Session::get('cart') as $key => $cartItem)
                                            ...
                                            if ($cart->variant != null) {
                                                $product_name_with_choice = $product->name.' - '.$cart->variant;
                                            }
                                            ...
                                            <tr class="cart-item">
                                                ...
                                                <td class="product-price d-none d-lg-table-cell">
                                                    <span class="pr-3 d-block">{{ format_price($cart->price) }}</span>
                                                </td>
                                                ...
                                                <td class="product-quantity d-none d-md-table-cell">
                                                    <div class="input-group input-group--style-2 pr-4" style="width: 130px;">
                                                        ...
                                                        <input type="hidden" name="product_id" id="prod_id" value="{{$cart->product->id}}">
                                                        <input type="text" name="quantity[{{ $key }}]" class="form-control h-auto input-number" placeholder="1" value="{{ $cart->quantity }}" min="1" max="10" onchange="updateQuantity({{ $key }}, this)">
                                                        <span class="input-group-btn">
                                                ...
                                                <td class="product-total">
                                                    <span>{{ format_price($cart->price * $cart->quantity) }}</span>
                                                </td>
...
@section('script')
    ...
    function updateQuantity(key, element){
        ...
        $.post('{{ route('cart.updateQuantity') }}', { _token:'{{ csrf_token() }}', key:key, quantity: element.value, product_id:product_id}, function(data){
            updateNavCart();
