...
@section('content')
    ...
    <section class="@if (!isset($type)) gry-bg @endif pt-5">
        <div class="container mb-4">
            ...
            <?php $user_id = Auth::id();?>
            @if(!in_array($id, $rated_seller) && $same_user == 0)
            <div style="padding:5px;" class="bg-white product-list row gutters-5 sm-no-gutters">
                <div class="col-md-6">
                <p>Rate Seller</p>
                <!-- <p>Your Submitted Review</p> -->
                <form action="{{ route('seller.rating', $id) }}" method="POST">
                @csrf
                <div class="row">
                    <div class="col-sm-6">
                        <div class="c-rating mt-1 mb-1 clearfix d-inline-block">
                            <input type="radio" id="star5_seller" name="rating_seller" value="5" required/>
                            <label class="star" for="star5_seller" title="Awesome" aria-hidden="true"></label>
                            <input type="radio" id="star4_seller" name="rating_seller" value="4" required/>
                            <label class="star" for="star4_seller" title="Great" aria-hidden="true"></label>
                            <input type="radio" id="star3_seller" name="rating_seller" value="3" required/>
                            <label class="star" for="star3_seller" title="Very good" aria-hidden="true"></label>
                            <input type="radio" id="star2_seller" name="rating_seller" value="2" required/>
                            <label class="star" for="star2_seller" title="Good" aria-hidden="true"></label>
                            <input type="radio" id="star1_seller" name="rating_seller" value="1" required/>
                            <label class="star" for="star1_seller" title="Bad" aria-hidden="true"></label>
                        </div>
                    </div>
                    </div>
                    <button type='submit' class="btn btn-danger mt-2">Submit Response</button>
                </form>
                </div>
            </div>
            @endif
            <div style="padding:5px;" class="bg-white product-list row gutters-5 sm-no-gutters">
                <div class="col">
                <p>What other people think about the seller</p>
                    <div class="rating clearfix d-block">
                    @foreach($review as $reviews)
                    @php 
                        $user = \App\User::where('id', $reviews->user_id)->first();
                    @endphp
                        <span class="star-rating">
                            @for ($i=0; $i < $reviews->rating_seller; $i++)
                                <i class="fa fa-star active"></i>
                            @endfor
                            @for ($i=0; $i < 5-$reviews->rating_seller; $i++)
                                <i class="fa fa-star"></i>
                            @endfor
                        </span>
                        <span class="ml-4">Rated By : {{ $user->name }}</span> <span class="text-secondary">({{$reviews->updated_at->format('Y-m-d')}})</span>
                        <hr>
                    @endforeach
                    </div>
                <div class="col-md-6">
