...
<title>@yield('meta_title', config('APP_NAME', 'Best E-commerce Worldwide Online Store By Women Sellers | SHEconomy'))</title>
...
@if(!isset($detailedProduct) && !isset($shop) && !isset($page))
    ...
    <meta name="twitter:title" content="{{ config('APP_NAME', 'SHEconomy') }}">
    <meta name="twitter:description" content="{{ $seosetting->description }}">
    <meta name="twitter:creator" content="@author_handle">
    <meta name="twitter:image" content="https://sheconomy.in/public/uploads/logo/sheconomy-og.jpg">

    <!-- Open Graph data -->
    <meta property="og:title" content="{{ config('APP_NAME', 'SHEconomy') }}" />
    <meta property="og:type" content="website" />
    <meta property="og:url" content="{{ route('home') }}" />
    <meta property="og:image" content="https://sheconomy.in/public/uploads/logo/sheconomy-og.jpg" />
...
<link type="text/css" href="{{ static_asset('frontend/css/magnific-popup.css') }}" rel="stylesheet" media="none" onload="if(media!='all')media='all'">
...
<link type="text/css" href="{{ static_asset('frontend/css/arrow-steps.css') }}" rel="stylesheet" media="all">
<link type="text/css" href="{{ static_asset('frontend/css/footer-collapse.css') }}" rel="stylesheet" media="all">
...
<meta name="google-site-verification" content="-NBqtpzczzj-BY1yV4J8jtUZZAjChrU5HUFYuYIcMtI" />
<!-- Global site tag (gtag.js) - Google Analytics -->
<script async src="https://www.googletagmanager.com/gtag/js?id=UA-180683682-1"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'UA-180683682-1');
</script>
</head>
<body>
<div class="multi_color_border fixed-top">
    <div class="container ">
        <div class="row">
            <div class="col-lg-8">
                <div class="multi_color_border" id="caption">
                    <p class="marquee">
                        <span>
            ...
            <div class="col-lg-4 ribbon-link">
                <a class="btn btn-danger" style="margin-left: 185px;" id="ribbon" target="_blank" href="https://community.sheconomy.in">Join Our Community</a>
            </div>
        </div>
    </div>
</div>
...
<script src="{{ static_asset('frontend/js/magnific-popup.js') }}"></script>
...
<script>
    ...
    function removeFromCart(key, product_id){
        $.post('{{ route('cart.removeFromCart') }}', {_token:'{{ csrf_token() }}', key:key, product_id:product_id}, function(data){
    ...
    var request_from = 'auto_checked';
        $.ajax({
            ...
            success: function(data){
                var variant =  $($.parseHTML(data)).find("#variant_exist").val();
                if(variant != null){
                    $('.showDiv').show();
                    $('#hideDiv').hide();
                    $('#status').html(data);
        ...
        if($('#option-choice-form input[name=quantity]').val() > 0 && checkAddToCartValidity()){
            $.ajax({
               ...
               success: function(data){
                   $('#hideDiv').hide();
                 // Variant Image End
                   $('#hide-price').addClass('d-none');
                   $('#option-choice-form #chosen_price_div').removeClass('d-none');
                   $('#option-choice-form #chosen_price_div #chosen_price').html(data.price);

                // $('#option-choice-form #chosen_price_div #chosen_price').html(data.variant_img);
    ...
    function addToCart(){
        if(checkAddToCartValidity()) {
            ...
            if (!loggedIn){
                showFrontendAlert('warning', 'Please Login Your Account!');
