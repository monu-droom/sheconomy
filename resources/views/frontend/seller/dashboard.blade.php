...
$shop = \App\Shop::where('user_id', Auth::id())->first();
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
           <form action="{{ route('dashboard.country') }}" method="POST">
            @csrf
                <div class="form-group">
                <label class="justify-content-center d-flex" style="font-weight: bold; font-size: 16px; text-align:" for="country">Please Select Your Country</label>
...
<!-- For Sell In Country modal -->
<div class="container">
  <!-- Modal -->
  <div class="modal fade" id="sell_in" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
           <form action="{{ route('dashboard.sell_in') }}" method="POST">
            @csrf
                <div class="form-group">
                <label class="justify-content-center d-flex" style="font-weight: bold; font-size: 16px; text-align:" for="country">Please Select Your Region To Sell Your Product</label>
                    <select name="sell_in" class="form-control" id="sell_in">
                        <option value="">--Want to sell in--</option>
                        <option value="india">India Only</option>
                        <option value="other">Globally</option>
                    </select>
                    <div class="text-center mt-2 ">
                        <button class="btn btn-danger">Submit</button>
                    </div>
                                ...
                                <div class="bg-white mt-4 p-5 text-center">
                                    <div class="mb-3">
                                        @if(Auth::user()->seller->verification_status == 0)
                                            <img loading="lazy"  src="{{ static_asset('frontend/images/icons/non_verified.png') }}" alt="" width="130">
                                        @else
                                            <img loading="lazy"  src="{{ static_asset('frontend/images/icons/verified.png') }}" alt="" width="130">
                                        @endif
                                    </div>
                                    @if(Auth::user()->seller->verification_status == 0)
                                        <a href="{{ route('profile') }}" class="btn btn-styled btn-base-1">{{ translate('Verify Now')}}</a>
                                    @endif
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="bg-white mt-4 p-4 text-center">
                                    ...
                                    <a href="{{route('get.payment.setup')}}" class="btn btn-styled btn-base-1 btn-outline btn-sm">{{ translate('Configure Now')}}</a>
                                </div>
                            </div>
                        </div>
                    </div>
