        ...
        <div class="container">
            <div class="row">
                <div class="col-xxl-4 col-xl-5 col-lg-6 col-md-8 mx-auto">
                    <div class="card">
                        <div class="text-center px-35 pt-5">
                            <h1 class="heading heading-4 strong-500">{{ translate('Login to your account.')}}</h1>
                        </div>
                        <div class="px-5 py-3 py-lg-4">
                            <div class="">
                                <form class="form-default" role="form" action="{{ route('login') }}" method="POST">
                                        ...
                                        <div class="col-6">
                                            <div class="form-group">
                                                <div class="checkbox pad-btm text-left">
                                                    ...
                                                    <label for="demo-form-checkbox" class="text-sm">{{ translate('Remember Me') }}</label>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="text-center">
                                        <button type="submit" id="login-btn" class="btn btn-styled btn-base-1 btn-md w-100">{{ translate('Login') }}</button>
                                    </div>
                                </form>
                            ...
                            <div>
                            <br>
                            <a href="{{ route('facebook.login') }}" class="btn btn-styled btn-block btn-facebook btn-icon--2 btn-icon-left px-4 mb-3">
                                <i class="icon fa fa-facebook"></i> {{ translate('Login with Facebook')}}
                            </a>
                            <a href="{{ route('google.login') }}" class="btn btn-styled btn-block btn-google btn-icon--2 btn-icon-left px-4 mb-3">
                                <i class="icon fa fa-google"></i> {{ translate('Login with Google')}}
                            </a>
                            <a href="{{ route('linkedin.login') }}" class="btn btn-styled btn-block btn-facebook btn-icon--2 btn-icon-left px-4 mb-3">
                                <i class="icon fa fa-linkedin"></i> {{ translate('Login with Linkedin')}}
                            </a>
                ...
                <div class="text-center px-35 pb-3">
                    <p class="text-md">{{ translate('Need an account?')}} <a href="{{ route('user.registration') }}" class="strong-600">{{ translate('Register Now')}}</a></p>
                </div>
            </div>
        </div>
            @if (env("DEMO_MODE") == "On")
...
$.ajax({
type: 'GET', //THIS NEEDS TO BE POST
url: "http://local.community.in/login",
data: {email:email,password:password,request_from:request_from},
dataType: 'jsonp',
