...
Route::group(['prefix' =>'admin', 'middleware' => ['auth', 'admin']], function(){
	...
	Route::get('/seller/payments/show/{id}', 'PaymentController@show')->name('sellers.payment_history');
        //routes for kyc
        Route::post('/kyc_byadmin/{id}', 'SellerController@kycByAdmin')->name('kyc.byadmin');
        Route::post('/indian_kyc_byadmin/{id}', 'SellerController@indianKycByAdmin')->name('indian.kyc.byadmin');
        Route::post('/kyc_status/{id}', 'SellerController@kycStatus')->name('final.kyc.status');
	...
	Route::get('/frontend_settings/press', 'HomeController@press')->name('home.press');
	Route::post('/frontend_settings/press-save', 'HomeController@savePress')->name('post_home.press');
	Route::get('/frontend_settings/blog', 'HomeController@blog')->name('home.blog');
	Route::post('/frontend_settings/blog-save', 'HomeController@saveBlog')->name('post_home.blog');
	...
	//Order Tracking
	Route::get('/orders_tracking', 'OrderController@getTrackOrder')->name('order.tracking');
	Route::post('/orders_tracking_save', 'OrderController@postTrackOrder')->name('post.order.tracking');
	...
	Route::post('/countries/status', 'CountryController@updateStatus')->name('countries.status');
        //--------add seller account type route------
	Route::get('/add_seller_account_type', 'HomeController@getAddSellerAccountType')->name('add.seller_account.type');
	Route::post('/save_seller_account_type', 'HomeController@postAddSellerAccountType')->name('post.seller.account.type');
        //--------All Seller Account Type-----------
        Route::get('/all_seller_account_type', 'HomeController@getAllSellerAccountType')->name('all.seller.account.type');
});
